...
**Real gap found and fixed same round (ADR-013 → ADR-014, `CP-003`):** `render-static.ts` was baking `state` from its `initial` value only — since `Drawer`/`Modal` gate their panel behind `state.open` (initial `false`), the static/`qa-html`/Astro output for both contained only the trigger button (confirmed by inspecting `compiler/out/{Drawer,Modal}/qa/*.html` directly), so the "0 violations" QA-001 result for both never actually audited the dialog panel. Fixed with `state[].staticValue` (`ast-component-spec.md` v1.3) — `astToMitosisComponent` gains a `forStatic` flag, `compile.ts` now builds a separate Mitosis component object for the static targets so `Drawer`/`Modal` bake `open: true` there while React/Vue keep the real `initial: false`. Re-running QA-001 against the now-visible panel immediately caught a real, previously-untested contrast bug: both components' `Close` button used `--br-color-secondary` background with white text for small text (`4.24:1`, below AA's `4.5:1`) — exactly the pairing `theme-spec.md` already warns is unsafe for small text. Fixed by switching to `--br-color-primary` (`6.57:1`); re-ran QA-001 clean, 0 violations **and** 0 unresolved `incomplete` color-contrast checks, across all 5 components plus a regression pass on `Button`/`Counter`/`SaveStatus`.

`Worklist.md`: `CMP-L1` → done, `CP-003` → done. `.specs/ADRS.md`: ADR-013, ADR-014. `.specs/SPECS.md`: indexed the new spec. `.specs/ast-component-spec.md` (v1.3) and `.specs/mitosis-compiler-spec.md` updated for `staticValue`/`forStatic`.
