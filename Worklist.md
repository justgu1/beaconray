...
| **QG-001** | Qualidade | Escrever `.specs/component-quality-spec.md`: gate obrigatório de HTML5 semântico, WCAG 2.1 AA, acesso multi-modal (teclado/voz/leitor de tela), orçamento de performance (ultra leve) e disciplina de tokens de animação. | HN-001 | Média | 🔄 Em andamento (spec escrita; subconjunto estático — tag semântica em evento, nome acessível, `alt`, `tabindex` — já é enforçado em código por `compiler/src/validate.ts`; `aria-live`/`prefers-reduced-motion`/foco continuam manuais, dependem de inspeção em runtime) |
...
*   **Total de Tarefas:** 17
*   **Concluídas:** 3 (HN-001, QG-002, CP-001)
