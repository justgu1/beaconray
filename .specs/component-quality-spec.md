...
- Dynamic content (depends on AST v1 — state) needs an `aria-live` region when the change isn't the direct result of the user's own action on that element. **Implemented (v1.2, heuristic)**: `validate.ts` requires `aria-live` on any state-bound node unless it's inside (itself or a descendant of) an element with any event binding — a deliberate simplification (exempting by "any event on an ancestor" rather than parsing exactly which state variable that event modifies, which would need real JS data-flow analysis). Covers the common case (a control that updates its own label) without a full expression parser; a state-bound display genuinely elsewhere in the tree (not a descendant of its trigger) still requires `aria-live`.
- Animation must respect `prefers-reduced-motion` (see rule 5) — not optional.
- Focus management inside conditionals (`Show`) is **implemented** — `focusOnShow` (`ast-component-spec.md` v1.2) moves focus to the block whenever it becomes visible, verified via Mitosis's dependency-tracked `onUpdate` hook (fires on every transition, not just initial mount). **Focus management for loops (`For`)** — what happens when a focused list item is removed — remains an open item: needs runtime tracking of "which item had focus, move to a sensible neighbor," not a simple dependency effect. Revisit with a real list component to test against, not in the abstract.
...
- **Structured data (schema.org JSON-LD)**: optional `schema` field on `ComponentAst` (`ast-component-spec.md` v1.2), emitted as a `<script type="application/ld+json">` block in the static output. **Implemented (SEO-002).** Validated to have `@context`/`@type` when present.
